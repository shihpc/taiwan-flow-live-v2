...
//
// 曝險盤點（前兩條已於 2026-09-12 的 B5 修掉，保留為變更紀錄）：
//   - ~~`num()` 對非數值字串回 NaN → 走「不可比」分支（後寫入者勝）~~
//     → 現在 NaN 不算有效，**有效值恆勝**，好點不再被字串壞點覆蓋。
//   - ~~`Number(null)`／`Number("")`／`Number(0)` 皆為**有限的 0**，會走正常比較並以最小值勝出
//     → amt 為 null/""/0 的壞點會鎖住該分鐘~~ → 現在一律視為無效，同樣由「有效恆勝」擋掉。
//   - **仍未解（上緣，與 B5 是相反的一端）**：09:00 那一分鐘可能拿到**前一交易日**的快照，
//     `amt` 因而是全日值（38 個歸檔日中 6 日中招＝15.8%，09:00 值 ÷ 前一交易日收盤值 ≈ 0.99）。
//     該分鐘**只被寫一次**，沒有更小的值可比，min 歸約與本次的有效性守門**都擋不住**；
//     要解得在寫入前驗快照日期（同 C6 的 `ts` 改造思路），牽涉 `data/intraday` 歸檔與
//     `rrg_base` 上游，屬跨檔變更、另案。
...
function pickSeriesDup(oldP, newP) {
  ...
  if (va !== vb) return va ? oldP : newP;
  if (!va || a === b) return newP;   // 兩者皆無效／平手 → 後寫者勝（沿用舊版同分鐘重跑冪等）
...
// 但若某日的較晚點被低報，會誤殺合法的較早點。實測 38 個歸檔日 10,448 對相鄰點有
// **47 對**違反（0.45%），其中 **46 對落在 09:00–09:05**，唯一例外是 07-29 的 09:40；
// 若良性違反普遍存在，全天都該散見才對。
// （**2026-09-12 覆驗更正**：本段原寫「17 對、全部集中在 09:00–09:03」——那是**動手前的
// 估計**，用 >10% 跌幅算的；重放實測後 commit message 與 PROJECT_SUMMARY 都更新了，
// 這段先寫好的註解卻沒跟著改，同一個 commit 內自相矛盾。）
//
// ⚠ **丟棄會串連**：一點只要比**任何**後續點大就丟，不只比相鄰點。故丟棄點（59）>
// 違反對（47），差額 12 點全部來自平台區（例 07-29 的 09:37–09:40 四分鐘同值 11904.9，
// 只有 09:40→09:41 構成相鄰下降對，其餘三點靠 minLater 串連被丟）。
// ⚠ **新增一項順序相依（在 B5 宣告的保證範圍外）**：同一個 `t` 被寫 ≥2 次、且其中一次
// `amt` 無效時，污染點被本函式刪除後就不再參與去重，結果會依寫入順序而異。B5 只保證
// 「amt 皆有限且兩兩相異」時順序無關，此情形需 amt 無效才可達（38 個歸檔日 10,486 點中
// 無效 amt **零筆**）。
// ⚠ **尚未線上驗證**：下一個可觀測點＝下一個交易日 09:00–09:05。`worker-deploy` 成功
// 只代表程式上線，不代表守門被觸發過。
...
// 19:07（＝主班 21:07 ＋10 小時）的依據（門檻值為 2026-09-07 使用者裁定、維持不變；三處門檻同步改。
// 下列數字是 2026-09-08 重算後的更正版，**取代 09-07 那組「120 筆／中位數 3h02m／p90 6h09m／
// 最大 9h45m／>4h 佔 26.7%／>10h 為 0 筆」——那組整組是錯的**，樣本漏抽本 repo 的
// daysummary.yml（cron 06:35Z，是家族內最早的 slot））：
//   更正後樣本＝四支 workflow（taiwan-flows/daily.yml、postmkt/build.yml、
//   taiwan-flow-live-v2/daysummary.yml、taiwan-stock-news/build-news.yml）2026-06-16~09-07
//   共 **187 筆** event:schedule run，created_at 減 cron 排定時刻——中位數 2h01m、p90 4h44m、
//   最大 **12h23m**；>4h 12.8%、>10h **2 筆(1.1%)**、>14h 0 筆。
//   結構性事實：GitHub 的延遲是**絕對排空時刻**、不是相對 slot 的固定倍數——2026-08-27／28
//   全平台塞車時四支的 created_at 黏在同一段絕對時刻，所以同一次事故下 slot 愈早、量到的
//   「延遲」愈大，「>10h 幾筆」取決於抽了哪些 workflow，不是平台常數（這正是上一輪出錯的機制）；
//   兜底班（23:07）在塞車事故中也幾乎沒有保護力，主班與兜底只會前後腳一起被釋出。
//   taiwan-backtest/walkforward.yml 自身只有 **4 筆**（workflow 2026-09-03 才建立，
//   延遲 3h13m~5h06m），**N=4 無法自證尾端**。紅線的理由**不是**「>10h 從沒發生過」
//   （該宣稱已被推翻），而是：三支 13:xx 班對同一 UTC 日 13:07Z 計延遲的**代理推估**
//   （N=151，**推測、非直接觀測**）誤報率約 2%（3 筆，全落在 08-27／28 單一次平台事故——
//   該事故 >+8h 共 6 筆、區間 9h51m~10h07m，其中超過 10h 的就是這 3 筆（10h00m~10h07m），
//   只超線幾分鐘、正好騎在 +10h 線上），換取距硬期限 **4h53m** 的人工補救餘裕；
//   對照 +4h（原門檻）代理誤報 16.6%、+8h 4.0%、+12h 0% 但餘裕只剩 2h53m。
//   另一側天花板是該腳本自己的硬期限：walkforward_daily.py 的 target=(now−12h) 在主班+14h53m
//   （台北隔日 12:00）後把參考日滾成隔天、該日永久漏記。
//   取樣限制：GitHub 只保留 90 天 run 紀錄，尾端只看得到 08-27／28 這一次事故，任何「最大＝X」
//   都是這次事故的函數而非平台上界；家族內尚有約 10 支帶 cron 的 workflow 未抽，「>10h 2 筆」
//   是下界不是全集。舊值 13:07 推自 walkforward.yml 檔頭「常態延遲 1~2 小時」那句，
...
export function backtestRefClock(tp) {
  ...
  const h = back ? tp.hour + 12 : tp.hour - 12;
...
// 從 CSV 檔尾文字撈「最後一筆的日期」（第 1 欄 date）。尾段可能從半列切起，故由後往前找第一個
// 以 YYYY-MM-DD＋逗號開頭的資料列——半列不會命中（切點在列中間），表頭 "date,..." 也不會命中。
export function extractTailDate(text) {
  const lines = String(text || "").split(/\r?\n/);
  for (let i = lines.length - 1; i >= 0; i--) {
    const m = /^(\d{4}-\d{2}-\d{2})\s*,/.exec(lines[i].trim());
    if (m) return m[1];
  }
  return null;
}
...
// 會持續成長的來源：Range 只取**檔尾**（後綴範圍 `bytes=-N`，RFC 7233 §2.1）。
// 已驗證事實（2026-09-07 curl 實打 ledger.csv 帶 `Range: bytes=-120`）：raw.githubusercontent.com
// 回 `HTTP/2 206` ＋ `content-range: bytes 55-174/175`，確認支援後綴範圍。用在 ledger.csv：它是 CSV 不是
// JSON、且每交易日追加一列會一直長，我們只要最後一筆，整檔抓下來沒有意義。
// N 的取法：ledger.csv 一列約 90 bytes（2026-09-07 實測全檔 175 B ＝表頭＋1 列），取 4096 B
// ≒ 最後 40 餘列，足以在任何情況下涵蓋「最後一筆」；檔案小於 N 時伺服器直接回整檔，成本上限
// 就是這 4KB。若 CDN 不支援後綴範圍而回 200 全檔，改用 reader 串流並只保留尾端 N 字元，
// 不把整包留在記憶體（與 fetchStatusHead 的 200 回退同一手法，方向相反）。
async function fetchStatusTail(fetchFn, url, bytes = 4096) {
  const r = await fetchFn(url, { headers: { Range: `bytes=-${bytes}` }, signal: timeoutSignal() });
  if (!r.ok && r.status !== 206) throw new Error(`HTTP ${r.status}`);
  if (r.status === 206 || !r.body || !r.body.getReader) return r.text();
  ...
  for (;;) {
    ...
    if (done) break;
    tail = (tail + dec.decode(value, { stream: true })).slice(-bytes);
  }
  return tail;
}
...
export async function buildStatus(env, tp, fetchFn = fetch, nowMs = Date.now()) {
  ...
  const defs = [
    ...
    { id: "postmkt", name: "盤後分析", grade: "market", due: STATUS_DUE_HOUR.postmkt, run: async () => {
    ...
    { id: "backtest", name: "策略回測", grade: "backtest", run: async () => {
      // CSV 不是 JSON、且逐日追加會一直長 → Range 只取檔尾 4KB（見 fetchStatusTail 註解）。
