...
5. **`/live` 的 `ts` 日期不可當資料日**（2026-08-30 實打再證）：週日 08-30 打線上 `/live` 得
   ...
   使用者已裁定要改語意，但**改法所需的量測在無 FinMind token 的環境做不到**（快照端點
   Sponsor 級且無歷史參數），且 `index.html:2485` 是 `if(j&&j.ts)` ——`ts` 回 `null` 會讓全站
   停在「載入中」。**動 `aggregate` 的 `ts` 前先讀** `PROJECT_SUMMARY.md`
   「/live 資料時間改取 max(date)」段的「使用者已裁定要改，但 2026-08-30 這次量測受阻」條。
