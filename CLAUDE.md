...
- **成本**：不多打任何一格 frame（只用迴圈裡已抓到的 54 格），只多一次 `GET /live`；體積**量測**
  **+33,080 bytes＝+11.2%**（294,570→327,650）——**不是端到端真跑**（該日 frame 已過 KV 2 天 TTL），
  量法＝真實歸檔檔＋按當日 `total` 比例重建的矩陣，見 `src/archive_intraday.py` 檔頭做法 4。
  覆驗者以 `agg_frame` 的真實算式獨立重核得 **+32,778（+11.1%）**，量級一致。
