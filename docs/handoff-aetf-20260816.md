...
**⚠ 上面第 55 行的成因敘述已過期**：本文寫的是「補位打破了前提」，但 2026-09-14 實查近 14 個交易日的 `data/aetf/2*.json`，`fallback_used` **全為 `None`**、超前四檔（`00400A`／`00407A`／`00987A`／`00996A`）的 `source` 皆為 `finmind`——補位早已不是分岔來源，成因變成 FinMind 對這四家的日期標記系統性快一天（9/14 天呈 16/4 分裂）。改口徑的結論不變，理由的內容變了。
