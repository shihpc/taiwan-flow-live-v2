...
## 0. 現況總覽（2026-09-10 更新，程式現值）

### 晚間場（pm）：`FX_ACTIVE_CARDS` 白名單 6 張 ＋ 長文卡 1 張
...
| `pm-aetf-2` | 主動ETF 動作總覽（**ETF 維度**，2026-09-10 就地改版）：一列一檔 ETF，主值＝該檔主動淨額、**排序鍵＝當日加減碼金額絕對值合計**（兩者刻意分家，見 §3B.2 末段），r2＝新/加/減/清四類檔數＋集保口徑規模；卡底揭露涵蓋率（納入 N 檔／M 檔當日無揭露／缺值筆數） | `fxCardAetf2` |
...
`PYTHONHASHSEED` 產出 byte-identical，見 §8；報告內段落＝M1/M3/M4/S1/S2/S3 等節名。
末欄一律寫報告的一級節代號，`backtest.yml` 的守門會逐列核對該節存在且仍有結論行）。
...
| 6 | 追高警示 | 爆量大漲 S≥2 R≥2% P≥0.7（排除漲停鎖死） | **不排序**，S 值降序呈現 | S3 |
...
**排序鍵與主值可以不同欄位（`pm-aetf-2`，2026-09-10 使用者裁示）**：該卡排序鍵＝當日加減碼
金額絕對值合計（Σ|val|），主值＝主動淨額（Σval）。理由是用淨額排序會讓「大買 A、大賣 B」的
ETF 正負相抵而排到後面，與「今天誰動作最大」的意圖相反。**代價是版型的金色比例條長度比例化
於主值、與名次順序不單調**（`render_ranking` 只認 `row.r`）——這正是 §3B.2 要求卡底標明排序
欄位的情境，該卡 note 同時寫明排序鍵與主值兩者的口徑。
**同批的誠實界線（2026-09-11 覆驗後補齊）**：①`val` 為 null 的加減碼以 0 計，**筆數必須寫在
卡底**、不得靜默；②**不放跨 ETF 共識**（「N 檔同買/同賣哪些股」）——資訊結構與 §3B.3 的
`flows-sync-1` 同型、零回測，屬 C 類，鐵律 #8 適用；③**兩個口徑差異必須揭露在 note**——
`r2` 的新/加/減/清取自上游 `k`，而 `k` 是**依原始股數**分類（`src/build_aetf_diff.py` 的
`# 持股型態分類` 段），與主動口徑的 `r` 在申贖日會系統性打架（15 天 273 個 ETF-day 中 13 個
`ratio`≠1，實例 `2026/08/04 00991A` 淨額 −8.9 億配「加19/減8」）；且「排除申贖」是**估算**
（FinMind 無總單位數，`units` 實查全為 null，一律走股數比中位數）。④**規模缺值時退的 `aum`
不是集保口徑**（成分股 market_value 加總，僅供估算，15 天 4/209 ETF-day 會走到），note 分開標。
⑤**比例條與名次不同軸必須點明**：`render_ranking` 的條長比例化於 `row.r`，全卡唯一沒被文字
說明的視覺元素恰好就是會誤讀的那個。
...
### 2026-09-10 加回一張：5 → 6 張（使用者裁示，G3）

`pm-aetf-2` **就地改版**（沿用 id 與 `FX_CARD_BUILDERS` 表位，builder 庫仍 35 張）：原「主動ETF
總覽」只有規模＋加減檔數、依規模降序——規模是慢變數、每天圖幾乎一樣，不符 §3C「更新頻率配得上
每日推播」。改為 ETF 維度的「動作總覽」後與個股維度的 `pm-aetf-5` 零欄位重疊、並存不取代。
同批為 aetf 三張卡（`pm-aetf-2/4/5`）補 **per-card 新鮮度守門** `fxAetfStale`：比對
`aetfDiff.primary_date`（正規化）與資料日 `baseline.date`，不符或無可信資料日一律 skip——
原本只有 `pushDailyCards` 的全域 `baseline.date` 閘門，aetf 管線單獨失敗時三張卡會帶舊
`primary_date` 照出（無聲降級）。
...
### 仍待處理（原兩條，其一已於 2026-09-14 結案；第二條仍未結案）

- ~~**`run_s2()` 沒有採用門檻。**~~ **✅ 2026-09-14 結案（使用者裁示 B1-1）**：
  `run_s2()` 已補門檻，**沿用 `M4_MIN_SPREAD`（0.30%）、非為本段新訂**——該常數訂於
  commit `cd3ba22`，早於 S2 這次檢視，因此不構成「看著 0.01% 回頭訂」的事後挑選。
  以報告記載的真實數字（分離度 0.01%、非單調）重放決策邏輯，結論行已由
  「排序欄位採用 **法人賣強度（越負越前）**」翻成「…未達先訂門檻 0.30%，卡5 不排序」；
  反向案例（分離度 0.92%）仍正常宣告採用，門檻不是一律否決。
  **本次不動 `backtest/report_sorting.md`**——`backtest/cache/` 不進 git（見下一條），
  離線重生不了，而結論行只能由腳本產出（`f46aaf8` 手改報告的前例就在檔頭），
  故報告內 S2 那行**要等下一次有 token／網路的重生才會翻面**，現行檔案仍是舊文案。
  生產端不受影響：卡5 本來就不排序（worker `fxCardExitSell`）。
  **範圍外、本批未動（順帶記錄，2026-09-14 覆驗補齊——原本只點名 `run_m1` 是漏列）**：
  與舊 `run_s2` 同形狀（`best = max(candidates, key=…)` 後無條件宣告採用、無門檻）的
  還有**兩處**——`run_m1()`（grep `def run_m1`）與 `run_s1()`（grep `弱勢榜排序欄位採用`）。
  `run_m3()` 走的是另一種判準（`Q1 med > 0`），不同型、不列入。本次僅依裁示處理 S2，
  這裡**不宣稱窮盡**，只記下已知的同型處。
