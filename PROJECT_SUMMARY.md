  ...
  - **三件既有狀況（非本批引入，僅記錄不修）**：
    ...
    - **「缺欄＝不可比」的舊敘述不精確**（複核者指出，已更正）：`Number(null)`／`Number("")`／
      ...
      **前一交易日**的快照，`amt` 因而是全日值。38 個歸檔日中 **6 日中招（15.8%）**，
      判準是 09:00 值 ÷ 前一交易日收盤累計額 ≈ **0.99**（實測 0.9557~0.9953，
      另一日因歸檔日有缺口而為 0.72）；該分鐘**只被寫一次**，沒有更小的值可比，
      **min 歸約與 B5 的有效性守門都擋不住**。與 C6 查到的 08:30 盤前殘留同一根因家族
      （台北 09:00:00 FinMind 仍在供昨日快照）。要解得在寫入前驗快照日期，牽涉
      `data/intraday` 歸檔與 `rrg_base` 上游，屬跨檔變更、另案。
