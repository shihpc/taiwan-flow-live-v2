  ...
  **亂序與重複點並不在被接受的範圍內**。
  ...
  - **三件既有狀況（其中兩件已於 2026-09-12 的 B5 修掉，保留為變更紀錄）**：
    ...
    - ~~**「缺欄＝不可比」的舊敘述不精確**：`Number(null)`／`Number("")`／`Number(0)` 皆為
      ...
      **仍未解（上緣，與 B5 相反的一端，2026-09-12 查出）**：09:00 那一分鐘可能拿到
      ...
      **①型（盤中漂移）仍未修，刻意的**：38 個歸檔日共 **17 對非單調**（累計額本該只增不減），
      分布 09:00(12)／09:01(3)／09:02(1)／09:03(1)。量級分兩種——②前日殘留是**巨大**
      （2.5–3.3 兆，6 對），①盤中漂移是**小幅**（約 30%，11 對，例 `2026-07-21 09:00 3768.5
      → 09:01 2385.5`），成因是「**今日**資料但取自**幾分鐘後**的快照，被寫進較早的標籤」。
      **不修的理由是沒有資料可以定門檻**（不是「影響很小」）：`stale` 的第二個條件是 >3 分，
      但先前觀測到 2026-09-11 的 frame 幾乎全帶 `stale:1`（時戳停滯型），用它當守門可能丟掉
      大量合法點。要定門檻得先量「合法漂移的分布」，那是另一批量測工作。
      ...
      **要解的程式改動其實很小**——`storeFrame` 在呼叫 `appendSeries` 前十幾行就已算出
      `stale`（含 `ts.slice(0,10) !== d`），照存不擋而已；**真正跨檔的是下游資料**
      （`data/intraday` 歸檔與 `rrg_base` 的既有形狀會變），故列為另案。
