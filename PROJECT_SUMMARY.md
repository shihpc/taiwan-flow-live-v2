...
- **🔬 台指期 tick 量測班上線（2026-09-09）——暫時班，只量測、不下判準**：
  ...
  ／`export function summarizeTickRows`，測試 `node worker/test/tickdiag.mjs`。
  規格與拆除條件寫在 `CLAUDE.md`「台指期 tick 量測班與 `/tickdiag`」節。
  - **動機**：未來若要把台指期 tick 接進 FinMind 哨兵，需要一個「日盤真的收完了」的落地判準。
    但**兩個直覺判準都已知不成立**——FinMind 日曆日 D 的 TX tick 檔**同時含三段**
    （D 的 00:00–05:00 夜盤延續／08:45–13:45 日盤／15:00–24:00 當日夜盤），於是
    ①「非空即落地」（`rows.length > 0`）**早在台北 09:00 就為真**，因為夜盤段先在檔裡 → 必假陽性；
    ②連 `max(time) >= "13:44"` 也不安全——**15:00 之後的當日夜盤列同樣滿足它**，
    等於把夜盤誤判成日盤收盤。要分辨只能看**分段**的列數與時間邊界。
  - **三個未知（本班要量的東西）**：
    1. 日盤收盤段（`time ∈ [13:44,14:00)`，樣本欄位 `d13`）**最早幾點拿得到**？
    2. **夜盤段是否先落地**（`seg.a`／`seg.c` 相對 `seg.b` 的出現順序）？
       ——**這一項需要上午的樣本**，也是 cron 從台北 09:00 就開始跑、而不是只跑傍晚的原因。
    3. 單日 payload 的**位元組數與 fetch+parse 耗時**（`bytes`／`clen`／`ms`）。
  - **⚠ 目前唯一的觀測：「台北 16:54 拿得到日盤收盤段」，N=1**。而且**那筆只存在於當時的對話裡，
    不在版控中**——沒有檔案、沒有 KV 樣本可回查，事後無法覆驗。所以它只能算轉述，
    不足以支撐任何判準，這正是要開這班去正式蒐集分布的理由。
    第一批**版控內／可回查**的樣本要等這班實跑（最快是上線後第一個交易日的台北 09:00–19:55），
    屆時 `GET /tickdiag?date=<當日>` 讀得回來。
  - **不變式**：本批**沒有動** `finFuturesVix`／`SENTINEL_SIGNALS`／`signalLanded`／`runSentinel`
...
- **✅ 已修並已驗證有效（修於 2026-08-09、部署 2026-08-10 凌晨 version `0ad82dc4` commit `ab8c766`、**2026-08-10 晚間以修正後第一個交易日的實資料驗證通過**）：`appendSeries` 的 lost update 競態（同日追查「07-30 series 少 1 筆」時發現）**：
  ...
  **亂序與重複點並不在被接受的範圍內**。
  ...
  - **三件既有狀況（其中兩件已於 2026-09-12 的 B5 修掉，保留為變更紀錄）**：
    ...
    - ~~`function num` 對非數值字串回 `NaN` → `pickSeriesDup` 走「不可比 → 後寫入者勝」
      → **壞點會覆蓋好點**~~ → **B5 已修**：`NaN` 現在算「無效」，由「有效值恆勝」擋掉，
      好點不再被字串壞點覆蓋。
    - ~~**「缺欄＝不可比」的舊敘述不精確**：`Number(null)`／`Number("")`／`Number(0)` 皆為
      **有限的 0**，會走正常比較並**以最小值勝出**（＝壞點鎖住該分鐘）~~ → **B5 已修**：
      三者一律視為無效。**「只有 key 根本不存在才不可比」這句話在 B5 後已不適用**——
      現在的分界是「有效（有限且 >0）／無效」，不是「可比／不可比」。
      **⚠ 原文此處曾寫「兩種行為都已在測試中釘住」，B5 後該句為假**（`worker/test/series.mjs`
      現在釘的是「好點恆勝」）——2026-09-12 覆驗退回並更正。
      **~~未解／待議~~ → 已做（B5，2026-09-12）**：原記錄「`amt <= 0` 是否該一併視為不可比」，
      當時未做的兩個理由中，**第二個已被實測推翻**——「盤前 09:00 的合法 `amt` 本就可能極小」
      不成立：38 個歸檔交易日、**10,486 個序列點，`amt` 為 0／負／null 的一筆都沒有**，
      且**每日最早時點都是 09:00、其 `amt` 最小值為 1,345.1 億**（開盤集合競價 09:00:00 即撮合，
      FinMind 當下回的累計額已是千億級）。**`amt<=0` 根本不需要門檻**——0 不是「極小」，
      而合法值的觀測下界離 0 有 1,345 億的距離。

      **實作上的關鍵（天真寫法會弄壞可交換性）**：判準寫成「**有效值恆勝無效值**」
      （有效＝有限且 >0），**不是**「無效 → 不可比 → 後寫者勝」。後者會讓 `{50,100,0}`
      依序寫入收斂到 `0`、其他順序收斂到 `50`，把本節花大篇幅建立的順序無關性打掉
      （同舊版 0.5 下限那版的病）。測試 `worker/test/series.mjs` 已加窮舉 3! 的守門。
      順帶關掉同段第一條曝險：非數值字串（NaN）現在同樣算無效，好點不再被字串壞點覆蓋。

      **上緣（與 B5 相反的一端，2026-09-12 查出）——②型已修、①型未修**：09:00 那一分鐘可能拿到
      **前一交易日**的快照，`amt` 因而是全日值。38 個歸檔日中 **6 日中招（15.8%）**。
      **判準的分母口徑要寫死＝前一交易日 series 的「末筆」（實際都是 `13:35`）**，
      比值 **0.9882~0.9953**（五日；第六日 08-03 為 0.7204，因真正的前一交易日 07-31 未歸檔）。
      ⚠ **初版寫的 0.9557 是錯的**（2026-09-12 覆驗抓出）：當時分母取「前一日 series 的 `max`」，
      而在被污染日的隔天，那個 `max` **正是前一日自己的污染點**（09-08 的 max 31727.6 就是它
      自己的 09:00，真末筆為 30494.4）——**拿污染值當基準去驗污染，是循環的**。
      **更硬的證據（覆驗者提出，優於比值）**：污染點的 `(idx, chg)` 與前一歸檔日收盤點**逐位元組相同**
      ——開盤不可能重現前日收盤指數到小數兩位＋同一個漲跌點。08-03 那日 `chg=3186.45`
      恰等於 `idx 43119.75 − 07-30 收盤 39933.3`，證明該快照屬 **07-31 盤**（未歸檔的真正前一交易日）。
      該分鐘**只被寫一次**，沒有更小的值可比，**min 歸約與 B5 的有效性守門都擋不住**。
      **✅ 已修（B5′，2026-09-12）**：`storeFrame` 在快照日期 ≠ 今日時**不寫序列點**
      （frame 本體與 `fi:` 索引照舊寫入、仍標 `_stale`），回傳值新增 `series_skipped` 供觀測。
      **守門刻意只綁日期那一半、不綁整個 `stale`**——`stale` 還含「時戳距牆鐘 >3 分」，
      那是延遲不是錯日；突變驗證：改綁整個 `stale` 會讓 4 條斷言轉紅（含 1 條既有測試）。
      **實害已確認並記錄、但不回填**：`data/daysummary/2026-09-04.json` 的
      `index.tse.lo = 45857.66`（＝前日收盤指數），當日真實低點 **46015.1**，**低了 157.44 點**；
      `src/build_daysummary.py` 的全日高低取自 series 的 `idx` max/min。六個污染日中只有這天的
      污染 idx 落在當日區間外，其餘五天**湊巧**落在區間內而沒影響高低點——**那是運氣不是防護**。

      **①型（盤中漂移）仍未修，而且它的實害比 ②型更大——2026-09-12 覆驗退回時才查出**：
      38 個歸檔日共 **17 對非單調**（累計額本該只增不減），分布 09:00(12)／09:01(3)／09:02(1)／09:03(1)。
      量級分兩種——②前日殘留**巨大**（2.5–3.3 兆，6 對），①盤中漂移**小幅**（約 30%，11 對，
      例 `2026-07-21 09:00 3780.2 → 09:01 2666.9`）。

      **三筆已落檔的線上錯誤（逐一實查，不是估計）**——判準＝污染點的 `idx` 落在當日其餘各點的區間外：

      | 日期 | 型 | 污染 `idx` | 當日其餘最低 | 誤差 | `daysummary` | B5′ 擋得到？ |
      |---|---|---|---|---|---|---|
      | 2026-07-21 | **①** | 42840.92 | 43015.04 | **−174.12**（B5″ 後為 **−232.08**，見註） | 該日無檔（最早 08-03） | ❌ |
      | 2026-09-04 | ② | 45857.66 | 46015.1 | −157.44 | `index.tse.lo` 已錯 | ✅ |
      | **2026-09-07** | **①** | **46724** | 46943.7 | **−219.70** | `index.tse.lo` 已錯 | ❌ |

      **07-21 那格兩個數字的口徑差**（2026-09-12 覆驗指出）：−174.12 是對照 09:01 的 43015.04，那是 B5′ 時代只看 09:00 一點的算法；B5″ 把 09:00 與 09:01 **都**丟了，對照點變成 09:02 的 43073，故回填試算得 **−232.08**。兩個數字各自正確，差在對照點不同。

      **最嚴重的那筆（09-07，−219.7 點）本次修法擋不到**。它不是前日殘留——其 `chg=172.87`
      的基準是 09-04 收盤 46551.13，屬當日資料；判它為污染的依據是**內部不自洽**
      （`amt` 3780.2 > 09:01 的 2666.9，累計額不可能下降）。

      **主對話第一版漏掉①型的實害，成因記在此**：對②型六天逐日跑了「污染 idx 是否落在當日
      區間外」的檢查，**卻沒對①型的 11 個點再跑一次同一個查詢**。資料與查詢都現成，純粹是
      「一類查得嚴謹、另一類直接跳過」。

      **成因假說要標為推測、不可寫死**（覆驗退回）：原寫「①型＝今日資料但取自**幾分鐘後**的
      快照」——以 `amt` 反查同日最接近時點的 `idx`，**11 點中有 4 點差 245–501 點**
      （07-21 −501.29、07-30 −392.53、09-07 −338.52、07-29 +308.53），該假說對它們**不成立**。
      目前能確定的只有「內部不自洽」，成因**未定**。

      **✅ 已修（B5″，2026-09-12，使用者裁示「做」）**：採用**免門檻的結構不變式**——
      累計額單調遞增，「較早時點的 `amt` 大於某個較晚時點」即判為污染（`export function
      dropNonMonotonic`，掛在 `mergeSeriesPoint` 排序之後）。對照組是「用 `stale` 的 >3 分」，
      那條需要一個沒有依據的門檻，且近 8 個交易日 `frames[].stale` 實測 **52–54/54**，
      綁整個 `stale` 會丟掉約 **98%** 的序列點。

      **實測血量（38 個歸檔日重放，不是估計）**：
      - 相鄰對 10,448 中**下降 47 對（0.45%）**，其中 **46 對落在 09:00–09:05**，
        只有 1 對在別處（07-29 的 09:40，該日是上游凍結日：09:33–09:36 四分鐘 `11672.3`
        完全相同、09:37–09:40 四分鐘 `11904.9` 完全相同）。**不變式是乾淨的。**
      - 丟棄點 **59 個 / 10,486（0.56%）**，涵蓋 29/38 日；多數日只丟 1–3 點，最大是 07-29 的 12 點。
        丟 59 > 違反 47 是因為**丟棄會串連**（一點只要比**任何**後續點大就丟）——這是
        「內容取自未來」這個機制的必然推論，但也是本做法最激進的地方。
      - **對交付物的實際影響只有 3 天**：`daysummary` 的高低點改變者僅 07-21／09-04／09-07，
        **全部是 `lo` 上修**（拿掉假低點），`hi` 一天都沒動；其餘 26 個有丟點的日子對高低點
        **零影響**。這 3 天正是上表那 3 筆已落檔的線上錯誤。

      **兩個必須寫明的取捨**：
      1. **丟「較早那一點」是判斷、不是推導**。依據是污染機制（快照取自未來），
         但若某日的較晚點被低報，會誤殺合法的較早點。07-29 丟 12 點就是這條的最壞情形
         ——該日本來就是上游凍結、資料不可信，接受。
      2. **無效 `amt`（≤0／非有限）的點不參與比較、也不更新 `minLater`**。否則中間一個
         `amt=0` 會讓**它之前所有點**被誤殺。突變驗證：拿掉這個豁免 → 4 條斷言轉紅，
         輸出顯示無效點之前的點全數消失。
      與 C6 查到的 08:30 盤前殘留同一根因家族（台北 09:00:00 FinMind 仍在供昨日快照）。
      **另有一型不可混為一談**（覆驗者附帶查出）：2026-07-29 的 09:00＝10303.3（約全日 30%），
      其 `chg=-64.19` 的基準是 07-28 收盤 → 那是 **07-29 盤的真值**、不是前日殘留，屬「遲到班把
      較晚的累計額寫進較早標籤」，同樣因單次寫入而擋不住，但成因不同。
      **~~故列為另案~~ 已於同日做掉（B5′）**——程式改動確實很小：`storeFrame` 在呼叫
      `appendSeries` 前十幾行就已算出 `stale`（含 `ts.slice(0,10) !== d`），照存不擋而已。
      **原寫「`rrg_base` 的既有形狀會變」經 grep 實查不成立**（2026-09-12 覆驗）：
      `src/build_rrg_base.py`／`src/build_rrg_frozen.py` 對 `series` **零命中**，
      它們吃的是 `frames`／`g`。真正變的只有 `data/intraday` 的 series 點數（276 → 275）。
...
- **/live 資料時間改取 max(date)（2026-07-21，deploy version `572399a2`）**：`aggregate()`
  ...
  **現況（2026-08-30 更新，原「未解/待觀察」已收斂成「已記載＋待裁決」兩半）**：
  ...
  - **✅ C 案已實作（2026-09-08）：`ts` ＝指數列時戳**——詳見本段末尾「✅ 已實作」區塊。
    以下三則（待裁決 → 量測受阻 → 量測管道）保留為決策過程紀錄，**其中「未做／待裁決」的敘述已作廢**。
  ...
  - **量測管道已建好：`/livediag` 唯讀診斷路徑（2026-09-05，本次上線；C 案本身仍未做）**——
    ...
    - **要觀察什麼／觀察到什麼才能決定改法**（下一個 session 照這張表做）：
      ...
      **📋 觀測紀錄（每採一次往下疊；湊齊後照上表決策）**

      | # | 採樣時點 | `ts_current` | `ts_le_close` | `ts_regular` | `ts_index.twse` | `classified`（n/pre_open/regular/late） | `hist` 交叉檢查 | 判讀 |
      |---|---|---|---|---|---|---|---|---|
      | 1 | **2026-09-05（週六）21:5x**〔非交易日〕 | `2026-09-05 08:30` | `2026-09-05 08:30` | **`2026-09-04 13:30`** | `2026-09-04 13:33` | 2787 / **2** / 710 / 2075 | **通過**：regular 時段 **181 個桶**、最大桶 `09-04 13:30` 345 筆（集合競價，合理）、次大僅 17 筆＝**不集中**；日期散在 08-31~09-04 五個交易日（冷門股最後成交日較舊，正常）＝**是真正的前一交易日** | **候選一給出正解**（週五收盤 13:30）；`regular=710>0` → **此形狀下 `ts` 不需回 `null`** |
      | 2 | **2026-09-06（週日）08:51**〔非交易日，前一日亦非交易日〕 | `2026-09-05 08:30` | `2026-09-05 08:30` | **`2026-09-04 13:30`** | `2026-09-04 13:33` | 2787 / **2** / 710 / 2075 | **通過**：257 桶，最大桶 `09-04 14:30` **1783 筆**（盤後定盤／零股）、次為 `09-04 13:30` 345 筆；最早桶 `08-07 14:59`（長期無成交的冷門標的） | **四個候選值與樣本 1 逐字相同**——`ts_regular` 在「前一日也是非交易日」的形狀下仍給出正解 |
      | 3 | **2026-09-07（週一）13:35**〔交易日，收盤直後、盤後定價未撮合〕 | `13:35:08` | `13:30:58` | **`13:30:58`** | `13:33:00` | 2787 / 2 / **2670** / 115 | 最大桶 `13:30`＝**2021**（集合競價收盤），最晚桶 13:35 | `latest` 前 8 全是 **`t=emerging`**（興櫃，交易到 15:00） |
      | 4 | **2026-09-07（週一）14:41**〔盤後定價交易已撮合〕 | `14:40:06` | `13:30:58` | **`13:30:58`**（與樣本 3 同） | `13:33:00` | 2787 / 2 / **726** / 2059 | 最大桶 `14:30`＝**1802**；`13:30` 桶由 2021 掉到 361 | 1802 ≫ 興櫃總數 539 → 該桶必含**上市櫃盤後定價交易** |
      | 5 | **2026-09-07（週一）21:01**〔全部盤後結束〕 | `15:00:00` | `13:30:27` | **`13:30:27`**（⚠ 較樣本 3/4 **早了 31 秒**） | `13:33:00` | 2787 / 2 / **696** / 2089 | `14:30`＝1802、`14:59`＝94、`13:30`＝359；最晚桶 `15:00` | `latest` 前 8 仍全是興櫃（15:00 收盤） |
      | **A** | **2026-09-08（週二）10:30**〔**盤中**〕 | `10:30:50.331812` | 同左 | 同左（盤中三者一致） | **`10:30:45`** | 2787 / 1 / 2680 / 106 | 最晚桶 `10:30`＝586，正在跳 | **指數列盤中隨盤跳動**，落後個股 max **5 秒** |
      | **B** | **2026-09-08（週二）11:31**〔**盤中**〕 | `11:31:19.519137` | 同左 | 同左 | **`11:31:15`** | 2787 / 1 / 2718 / 68 | 前 3 桶 `11:29`/`11:31`/`11:30`，持續遞增 | 落後 **4 秒**，與樣本 A 一致 |
      | 6 | **2026-09-08（週二）21:01**〔交易日 2，全部盤後結束〕 | `15:00:00` | `13:30:57` | **`13:30:57`**（在 13:30 桶 ✓）| **`13:33:00`**（與 09-07 逐字相同）| 2787 / 1 / 710 / 2076 | `14:30`＝1780、`13:30`＝363、`14:59`＝92；最晚桶 `15:00` | 與交易日 1 幾乎同形（前日 1802／359／696）；`ts_index` 連續兩個交易日都是 `13:33:00` |
      | 7 | **2026-09-09（週三）21:01**〔交易日 3；**C 案上線後首次完整交易日**〕 | `15:00:00` | `13:30:45` | **`13:30:45`**（13:30 桶 ✓）| **`13:33:00`**（連續第 3 個交易日同值）| 2786 / 1 / 802 / 1983 | `14:30`＝1694、`13:30`＝436、`14:59`＝96 | **回歸監控全綠**：`/live` 的 `ts`＝`2026-09-09 13:33:00`（指數列）、`snap_ts` 未外洩、`degenerate=true`、`frames={10:13:35,30:13:35}`（證明 `pickFrames` 仍吃 `snap_ts`）|
      | 8 | **2026-09-10（週四）21:02**〔交易日 4〕 | `15:00:00` | `13:30:55` | **`13:30:55`**（13:30 桶 ✓）| **`13:33:00`**（連續第 4 個交易日同值）| 2786 / 1 / 766 / 2019 | `14:30`＝1724、`13:30`＝398、`14:59`＝95 | 回歸監控全綠：`/live` `ts`＝`2026-09-10 13:33:00`、無 `snap_ts`、`degenerate=true`、`frames` 仍 `13:35`、`flow_last`＝`2026-09-10`/`13:33:00`（新語意穩定第 2 天）|
      | 9 | **2026-09-11（週五）21:04**〔交易日 5，**判準表要求的最後一個交易日樣本**〕 | `15:00:00` | `13:30:57` | **`13:30:57`**（13:30 桶 ✓）| **`13:33:00`**（連續第 5 個交易日同值）| 2786 / 1 / **799** / 1986 | `14:30`＝1701、`13:30`＝426、`14:59`＝91；最晚桶 `15:00` | 回歸監控全綠：`/live` `ts`＝`2026-09-11 13:33:00`、payload **無 `snap_ts`**、`flow.degenerate=true`、`flow.frames`＝`{10:13:35, 30:13:35}`（證明 `pickFrames` 仍吃 `snap_ts`）、`flow_last.date`＝`2026-09-11`／`ts`＝`13:33:00` |

      **樣本 1 的關鍵發現（改變了問題的形狀）**：`pre_open` **只有 2 列**。
      整包 2787 檔中 710 檔保留真實正規盤時戳、2075 檔是盤後（最大桶 `09-04 14:30` 1783 筆＝零股）。
      **不是「所有列被改寫成 08:30」，而是「2 列脫隊的 08:30 毒死了 `max(date)`」**——
      比原本的假設精確得多。另：`ts_le_close` 與 `ts_current` 同值，**實證字面 `≤13:30`
      單邊門檻濾不掉 08:30 殘留**（原派工規格的錯誤，已由 `ts_regular` 的 09:00 下界修正）。
      再另：**兩個候選不一致**——`ts_regular` 13:30 vs `ts_index` 13:33，差 3 分鐘
      （指數列在集合競價結算後才更新），選哪個要一併裁示。

      **樣本 2 的關鍵發現：毒源身分已確認（`latest` 欄直接點名）**。那 2 列 `pre_open`
      是 `020011`（統一微波高息20N）與 `708785`（信驊中信9B購01）。查 `data/classify.json`
      的 `map`：

      | 代號 | 名稱 | `e` 產業別 | `c` 產業鏈 | `sh` 發行股數 |
      |---|---|---|---|---|
      | `020011` | 統一微波高息20N | **ETN** | `[]` | 0 |
      | `708785` | 信驊中信9B購01 | **所有證券**（權證） | `[]` | 0 |
      | （對照）`2330` | 台積電 | 半導體業 | `["半導體"]` | 25,932,370 |

      兩者都在 `map` 內，所以被 `/live` 算進 `classified`，`max(date)` 就被它們拉走。
      **這給了 (乙) 案一條比時窗更精準的濾法**，但要用對欄位——實測 `classify.json`（3147 檔）：

      - **`sh == 0` 不可當判準**：781 檔命中，含 106 檔生技醫療業、51 檔半導體業等**真個股**
        （只是沒有持股申報資料），會誤殺一大票。
      - **`e` 產業別可以**：`ETN` 28 檔、`所有證券`（權證）36 檔、`Index`（指數列）30 檔，
        合計 94 檔＝3.0%，其中 `sh>0`（可能是真個股）者 **0 檔**，是乾淨的封閉集合。

      即 **(乙) 案候選：`ts` 只取 `e ∉ {ETN, 所有證券, Index}` 的列**。相對 `ts_regular` 的好處是
      不必猜時窗邊界，也不會把 14:30 的盤後定盤一起排除——**排除盤後定盤是「語意選擇」，
      不該混進「濾雜訊」這一步**（`hist` 顯示 late 2075 列裡 1783 列集中在 `14:30`，
      那是真實的盤後定盤／零股成交，不是雜訊）。

      **兩樣本都未能回答的**：交易日收盤後，**真個股**會不會自己產生脫隊時戳
      （處置股分盤交易、暫停買賣後殘留等）。若會，光濾券種不夠、仍須配時窗。

      **樣本 3–5 的三個結論（2026-09-07 交易日全程追蹤，判準表兩題已可回答）**

      **① 雜訊有三個來源，不是一個。** 原先以為只要濾掉 ETN／權證／指數列就夠，實測推翻：

      | 來源 | 規模 | 為什麼會推進 `max(date)` | 只能用哪個欄位濾掉 |
      |---|---|---|---|
      | ETN／權證／指數列 | `e` 為 `ETN`(28)／`所有證券`(36)／`Index`(30)＝94 檔 | 非個股，時戳自成一格（樣本 2 的 08:30 殘留） | **`e` 產業別**（`t` 濾不掉：`020011` 是 `t=twse`、`708785` 是 `t=tpex`）|
      | **興櫃** | `t == "emerging"` **539 檔** | **真個股、真成交**，但交易時段到 **15:00** | **`t` 市場別**（`e` 濾不掉：它們的 `e` 都是正常產業別如生技醫療業、電機機械）|
      | **上市櫃盤後定價交易** | 樣本 4/5 的 `14:30` 桶 **1802 筆** | 14:00–14:30 收單、**14:30 撮合**，是上市櫃自己的合法成交 | **只能靠時窗**——兩個欄位都濾不掉（它們就是 twse/tpex 的正常個股）|

      `map` 市場別分布：`twse` 1544／`tpex` 1064／`emerging` 539。1802 ≫ 539，故 `14:30` 桶
      至少 1263 筆是上市櫃盤後定價交易，**這是「時窗上界不可省」的直接證據**。
      → **(乙) 案的正確條件是三者並用**：`t ∈ {twse, tpex}` **且** `e ∉ {ETN, 所有證券, Index}`
      **且** 時窗 `09:00`–`13:30`。（前兩者是為了讓時窗上界不必為興櫃/ETN 的怪時戳背書，
      不是可以取代時窗。）

      **② 資料列是覆寫、不是追加**——這是先前沒有意識到的機制。同一檔的列會被盤後成交
      **改寫時戳**：`regular` 2670（13:35）→ 726（14:41）→ 696（21:01），
      `13:30` 桶 2021 → 361 → 359。含意是 **`ts_regular` 的穩定性不是結構保證**：
      它取的是「當下仍留在時窗內的列」的 max，而那一列隨時可能被盤後成交搬走。

      **③ 因此 `ts_regular` 分鐘級穩定、秒級會漂**（本次實測）：
      `13:30:58`（13:35）→ `13:30:58`（14:41）→ **`13:30:27`（21:01，往前掉 31 秒）**。
      最大值那列被盤後成交覆寫後，max 退到次大值。三次都仍落在 **`13:30` 這一分鐘內**，
      所以以「分鐘」為精度是穩的，以「秒」為精度不是。

      **判準表第一題（`ts_regular` 是否落在 13:2x/13:30）→ 是，三次皆是**，
      且 `regular > 0`（696–2670）→ 依判準表，**(甲) 改法＝候選一（時窗過濾取 max）成立，不必回 null**。

      **判準表第二題（`ts_regular` vs `ts_index` 是否一致）→ 不一致，但差異穩定**：
      `ts_index` 三次**完全不動**（`13:33:00`，twse／tpex 同值），`ts_regular` 則在 `13:30:2x`–`13:30:5x`
      之間漂。判準表原訂「差 <1 分即候選二可用」——實測差 **2.5～3 分鐘**，超過門檻。
      **但這裡有個判準表沒預料到的反轉**：`ts_index` 之所以不動，正是因為**指數列不參與個股盤後交易**，
      結構上免疫於 ②的覆寫機制；`ts_regular` 反而是那個會漂的。
      **兩者誰更適合當「資料時間」是語意選擇，不是精度問題**——
      `13:30` ＝ 個股正規盤最後成交、`13:33` ＝ 指數收盤結算完成時點。**這項仍待使用者裁示，本文不代決。**

      **仍缺**：交易日樣本目前只有 **1 天**（判準表要求 5 個交易日）。今日三筆已足以確立
      *機制*（三個雜訊源、覆寫行為、秒級漂移），但「`ts_regular` 是否每天都落在 13:30 桶」
      仍需再累積 4 個交易日；特別要留意**提早收盤日與颱風假**這類非典型交易日。

      **✅ 採樣結案：五個交易日全數到齊（2026-09-11 樣本 9，09-07~09-11 每個交易日各一筆收盤後樣本）**

      判準表要求的三題，逐題以實測回答：

      | 判準表的問題 | 五日實測 | 結論 |
      |---|---|---|
      | `ts_regular` 是否每天都落在當日 `13:30` 桶？ | **5/5 是**：`13:30:27`(09-07)／`13:30:57`(09-08)／`13:30:45`(09-09)／`13:30:55`(09-10)／`13:30:57`(09-11)，全部在 `13:30` 這一分鐘內；同期 `regular` 為 696／710／802／766／799，**每天都 >0** | **候選一（時窗 max）在五個交易日上成立，且不必回 `null`** |
      | 秒級漂移的幅度範圍？ | **跨日**：`:27`~`:57`＝**約 30 秒**跨度（**表上為秒級截斷值**，原始帶毫秒——09-11 實打為 `13:30:57.359000`，故真值落在 29.4~30.4 秒之間，「30」不是精確值）。**同日內**（唯一有日內序列的 09-07）：`13:30:58`(13:35)→`13:30:58`(14:41)→**`13:30:27`**(21:01)＝**往前掉 31 秒** | 分鐘級穩、秒級不穩。**同日內的漂移不是雜訊而是機制**——盤後成交覆寫掉原本的 max 那列，max 退到次大值（見上方結論②） |
      | `ts_index` 是否每天都是 `13:33:00` 不動？ | **`twse` 欄 5/5 是**；09-07 當天 13:35／14:41／21:01 三次取樣也**逐字相同**。**`tpex` 同值只有 3/5 有依據**（09-07 見下方判準表第二題段、09-08 見部署後 A/B 實查表、09-11 為覆驗時實打）——**觀測表只有 `ts_index.twse` 一欄，09-09／09-10 兩天沒有 tpex 紀錄**，不可宣稱 5/5 同值 | **跨日與日內雙重不動**。結構性理由：指數列不參與個股盤後定價交易，免疫於覆寫機制 |

      **非典型交易日（提早收盤、颱風假）仍未取得樣本**——這五天都是正常交易日。
      這是**取樣限制、不是已驗證的結論**：`13:33` 是否隨提早收盤而前移，本批無法回答。
      影響有限（`ts` 只用來標示資料時間，前端 `liveDataDate` 只取日期部分，`dataDateFromTs`
      的門檻是 `≥09:00`，提早收盤的結算時戳仍會遠高於它），但不可宣稱已驗證。

      **決策欄（本表原設計是「湊齊後照上表決策」）——決策已於採樣完成前做出**：
      使用者 2026-09-07 裁示採候選二（指數列），C 案已於 2026-09-08 **台北 23:07** 上線
      （`7f58829`，version `aba39e9e`，部署時刻見下方「部署後線上實查」段）。
      **樣本 1–6＋A、B 全部早於上線**（樣本 6 是 09-08 **21:01**，比部署早約 2 小時），
      **只有樣本 7–9 是上線後的回歸監控，三筆全綠**（`ts` 為指數列、`snap_ts` 未外洩、
      `pickFrames` 仍吃 `snap_ts`）——樣本 6 那一列也確實沒有這四項欄位。
      09-08 當天的上線後直證不在觀測表，而在下方台北 23:08 的 A/B 實查表。
      **「不是決策輸入」對樣本 6 仍成立**（裁示在 09-07，早於它）。
      事後看，五日數據**支持**該裁示：會漂的是候選一、不動的是候選二。

      **✅ 使用者裁示（2026-09-07）：`ts` 改用候選二＝指數列（`13:33`）。**
      理由（使用者原話）：「13:33＝指數收盤結算完成時點（完全不漂）」——即取其
      **結構性穩定**，而非取精度。這與樣本 3–5 實測一致：指數列不參與個股盤後交易，
      免疫於「資料列被覆寫」機制，五個樣本 `ts_index` 一次都沒動過；`ts_regular` 則秒級會漂。

      **✅ 缺口已關閉（2026-09-08 盤中兩筆實測）**：原本擔心「指數列只在收盤結算時寫一次、
      盤中會凍住」——**證偽**。盤中兩次取樣 `ts_index` 分別為 `10:30:45`／`11:31:15`，
      **穩定落後個股 max 僅 4–5 秒**，確實隨盤跳動。
      → **乙案三種時段全線成立**：盤中隨盤跳、收盤後鎖 `13:33`（五次不動）、
      非交易日給出正確的前一交易日。使用者裁示的理由（結構性穩定）三個時段都站得住。
      ...
      **✅ 已實作（2026-09-08，commit `7f58829`，`worker-deploy` run `34242689680` success，
      ...
      > `ts` ＝指數列 `001` 的 `date`；`001` 缺失退 `101`；兩者皆缺回 `null`；
      > `001` 與 `101` 不一致時取 `001`（理由：前端頂列與 `/status` 的 live 都是加權指數口徑）。

      實作細節與刻意的取捨：

      - **`snap_ts` 逐字不動**：`aggregate` 內原本那行 `if (r.date && (!ts || r.date > ts)) ts = r.date;`
        **保留不刪**，它是 `snap_ts` 的來源；只有 return 的 `ts:` 換成新算出的 `idxTs`
        （`ts: idxTs, snap_ts: ts`）。`worker/test/snapts.mjs` **未修改、21 條全綠**
        ——該檔手工組 live 物件、不呼叫 `aggregate`，故語意分家不影響它。
      - **「缺失」的判定放寬到「date 不可用」**：`001` 列缺席、或該列沒有 `date`／`date` 為空字串，
        一律往下退到 `101`；退無可退才 `null`。裁示只說「`001` 缺失退 `101`」，
        「有列但無 date」對取值而言與缺席等價，不另立分支（helper＝`const idxDate`）。
      - **新測試 `worker/test/livets.mjs`（21 條）**：涵蓋裁示的四種形狀（①`001` 在 ②`001` 缺退 `101`
        ③兩者皆缺 `null` ④`001`≠`101` 取 `001`），並在每種形狀同時釘住 `snap_ts` 維持舊口徑；
        另加「列順序顛倒仍取 `001`」「`101` 較晚仍取 `001`」兩條，把規則與
        「取第一列指數」「取兩者較大者」兩種誤實作分開；再加 date 空字串／無 date 欄／
        無有分類個股／非交易日殘留四組邊界。
      - **既有測試改了兩條斷言（皆在 `worker/test/livediag.mjs`，皆是語意變更的必然後果）**：
        ①「`aggregate` 的 `snap_ts` 與 `ts` 同值（拆語意不拆值）」——**前提已不成立**，
        換成三條更強的：`snap_ts` ＝ max(date)、`ts` ＝ `001` 的 date、兩者確實不等。
        ②「路由 `/live` 的 `ts` 仍是原口徑 max(date)」——該條的存在意義就是釘住舊語意，
        改成釘住新語意（＝`001` 的 date）＋「明確不是 max(date)」，並補一條
        「`/livediag` 的 `ts_current` 仍是 max(date)」保住診斷對照組。
        兩處都是**把弱斷言換成強斷言**，不是放寬。其餘 22 支測試零修改、全綠。
      - **下游三處實查結論（非臆測）**：
        | 下游 | 結論 |
        |---|---|
        | `export function flowLastPayload` | **無誤**。寫入窗 13:25–13:40 落在盤中／收盤直後，指數列此時必為當日值（09-08 盤中兩筆實測隨盤跳、09-07 13:35 實測已是當日 13:33:00），故 `flow_last.date` 不會被寫成昨日——原註解裡「C 案若採指數列可能寫成昨日」的風險**前提已被證偽**，註解同批更新。`ts` 為 `null` 時 `String(live.ts \|\| "")` 已守、**不拋錯**，只是 `date` 會是空字串（需兩列指數皆缺，屬上游嚴重劣化；前端 `liveDataDate` 對非 `YYYY-MM-DD` 的 `flow_last.date` 有正則守門，不會誤顯）|
        | `src/build_daysummary.py:141` | **仍正確**。閘門是 `(live.ts或"")[:10] != date → exit 0`；主觸發 Worker 13:35／備援 cron 14:35 台北，兩者都在 13:33 之後 → 交易日 `ts` 日期＝今日、閘門通過（與舊語意同結果）。非交易日新語意給前一交易日 → 仍不等於目標日 → 照舊優雅退出。**風險形狀改變但未擴大**：`ts` 為 `null` 會讓閘門 exit 0、daysummary 斷（連鎖 LINE ov 卡與 taiwan-stock-news「昨日市場」）——舊語意下「全體分類失敗」也會 `ts=null`，只是觸發條件換成「兩列指數皆缺」|
        | 前端 `function liveDataDate`（`index.html`，**本批未改**）| **新語意下行為正確**。非交易日 `ts` ＝前一交易日 `13:33`（≥09:00）→ 直接走第一分支取其日期＝正解；`hm<09:00` 的 `flow_last`／`prevWeekday` 後備如預期**走不到**（無害，留作 `ts` 形狀劣化時的降級）——**2026-09-08 覆驗更正：當時寫成「變成死碼」是過強的斷言**。後備分支整體是**活路徑**（`ts` 為 `null` 時就會走到它，見本列下一句），走不到的只有「`ts` 存在但時分 <09:00」這個子情形，**且那是推論不是保證**（依據＝下方「觀測紀錄」表 8 筆樣本 #1–#6 ＋ A、B 的 `ts_index` 全部 ≥09:00，無 <09:00 樣本；**不是**舊語意那兩筆 08:30 殘留觀測）。`ts === null` → 正則不匹配 → 回 `flow_last.date` 或 `null`，`renderTopline` 顯「—」、`liveStatus` 走「休市定格」，**不拋錯**。**原留給前端那批的兩個文案問題已於 2026-09-08 修畢（commit `af03a9e`＋覆驗退回修正）**：`liveStatus` 的 tooltip 曾寫「最後成交 ${live.ts}」、且 `ts` 為 `null` 時會印出字面 `null`（僅當 `flow_last.date` ＝今日才可能同時成立）——現改「指數時間 ${idxTsText(live)}」，`null` 降級為「—」|

      **✅ 已結案：前端那批的文案（2026-09-08，commit `af03a9e`＋覆驗退回後的修正）**。原清單實查
      `index.html` 消費 `live.ts` 共 **8 處**，其中 4 處要改、4 處不必改，現況如下（行號為改前）。

      | 位置 | 原問題 | 結果 |
      |---|---|---|
      | `:332` `renderTopline` 資料日 tooltip | 寫「最後成交 `${live.ts}`（上游時戳，**非資料日依據**）」——新語意下自我矛盾，因為 `liveDataDate` 正是拿 `ts` 的日期當資料日 | ✅ 改由 `function dataDateTitle` 產生三態文案（見下方「教訓」）|
      | `:873` 摘要分析 crumb（可見文字） | 「最後成交 `${state.live.ts}`」 | ✅ 改「指數時間 …」，並補 `title` 澄清「非個股最後成交時間」|
      | `:322`／`:323` `liveStatus` tooltip | 寫「最後成交」；且 `ts` 為 `null` 時印字面 `null` | ✅ 改「指數時間 `${idxTsText(live)}`」，`null` 降級為「—」|
      | `:685`／`:1021`／`:1441`／`:2411` | **無正確性問題**——都取日期部分、都有 `||""` 守門，且新語意下更正確（例：`:2411` 的 `/replay?date=` 舊語意在週末會拿週六日期去打） | ✅ 依判斷未動 |

      **實際措辭決定**：畫面用詞統一為「**指數時間**」；crumb 另掛 `title`
      「加權指數（缺則櫃買指數）最後更新時刻，**非個股最後成交時間**」，避免使用者把它讀成個股最後一筆成交。
      頂列資料日 tooltip 寫「指數時間 `<ts>` —— 加權指數（**缺則櫃買指數**）更新時刻，資料日即取自此」，
      其中「資料日即取自此」是**有條件**的（`ts` 時分 ≥09:00 才成立，由 `function dataDateFromTs` 判定，
      與 `liveDataDate` 共用 `TS_RE`／`function tsParts`）；走後備分支時改說「**但資料日未取自它**」——
      句尾**到此為止、不多描述一個字**（兩次覆驗連續退回同一型）：不提「早於 09:00」（否定側也涵蓋
      「`ts` 為 truthy 但格式不合、`TS_RE` 沒解析成功」，那條從未做過該比較），也不說「改由後備來源推得」
      （後備可能落空——無 `flow_last`、或日期部分不可 `Date.parse` 使 `prevWeekday` 回 `null` ——
      此時資料日顯「—」，根本沒推得任何東西）。後備成不成功，畫面上的「資料日」欄自己會說。
      規範正本記在 `CLAUDE.md`「頂列」節。

      **⚠️ 教訓（本批覆驗退回一項，值得記住）**：這批的存在理由就是消滅「敘述與程式不符」的文案，
      結果第一版**新造了同型矛盾**——資料日 tooltip 寫成 `live && live.ts ? A : B`，
      **三元的 falsy 分支把兩種完全不同的狀況合併處理**：`live` 為 `null`（`/live` 兩次都沒拿到，
      `boot` 仍會呼叫 `renderTopline`）與「拿到 payload 但 `ts` 為 `null`」共用了否定分支，
      於是 `/live` 掛掉時 tooltip 顯示「無指數時間（上游未提供加權／櫃買指數列）」，
      **斷言了我方根本不知道的上游成因**，且與同一列右側狀態欄「`/live` 載入失敗，無法判斷資料狀態
      （不代表資料異常）」直接矛盾；舊版此路徑的 title 是空字串，所以還是**行為退步**。
      根因：**「我不知道」與「我知道它是空的」不是同一件事，別讓它們共用同一個 else**。
      現改為三態函式 `function dataDateTitle`（`live` 為 null → 空字串、不掛 tooltip、不臆測成因）。

      **另補**：daysummary 的觸發時點除主觸發 13:35／備援 cron 14:35 外，Worker 另有 **14:05 recheck**
      （`worker/src/index.js` 的 recheck 分支）。且「13:35 時指數列必為當日」的依據是**單次觀測**
      （09-07 13:35）＋盤中兩筆隨盤跳的推論，有 14:05／14:35 兩層兜底，風險可接受。

      **理論邊界（不構成缺陷）**：`idxDate` 以 `r.date` 判真假，純空白字串 `"   "` 會被當可用值。
      FinMind 不曾出現此形狀。
      ...
      **`/livediag` 的 `classified` 只看時分、不看日期（讀數時要記得）**：盤中 10:30 竟有
      ...
      不是今天有盤後交易。不影響 `ts_regular`（取 max，今日的贏過昨日的），但別把
      `classified` 的計數當成「今天的分佈」。

      **實作面的三個接觸點**（改 `ts` 前先讀，行號為 2026-09-07 實查）：

      | 位置 | 現況 | 改 `ts` 的影響 |
      |---|---|---|
      | `worker/src/index.js:195` | `aggregate` **刻意跳過**指數列（`if (code === "001" \|\| code === "101") { idxrow[code] = r; continue; }`）才算 `ts` 的 max | **這就是改法的落點**：改成取 `idxrow["001"].date` |
      | `:736` | `date: String(live.ts \|\| "").slice(0, 10)`——**直接從 `ts` 切日期** | 連帶被改，需一併確認語意仍正確 |
      | `:126` | `d` 取「第一個非指數、有分類個股」的 `date`（獨立路徑，不經 `ts`） | 同族問題，改 `ts` 時應一併檢視是否要對齊 |

      另需訂**指數列缺失時的退路**（`idx["001"]` 為 `null`）與 **twse／tpex 不一致時取哪個**
      （五個樣本兩者皆同值，但沒有規則就是隱性依賴）。

      **📐 三案並列評估（2026-09-07，使用者追加要求評估混合案後的唯讀評估）**

      | | **甲** 時窗 max | **乙** 指數列 `001` | **丙** 混合（盤中甲／收盤後乙）|
      |---|---|---|---|
      | 語意一句話 | 上市櫃普通股正規盤 09:00–13:30 最後成交時戳 | 加權指數最後結算時戳（收盤日 13:33）| **寫不出一句話**——同一欄位一天兩種定義 |
      | 分支 | 無（純過濾＋max）| 無，但要訂 `001` 缺失／`001≠101` 退路 | 有，且切換依據無良解 |
      | 非交易日 | 樣本 1/2 實測給正解 `09-04 13:30` | 樣本 1/2 實測給正解 `09-04 13:33` | 週六上午判「盤中」走甲、下午走乙＝**同日兩種語意** |
      | 秒級穩定 | 會退 31 秒，分鐘級穩 | 五樣本一次沒動 | 切換點跨分鐘退 2.5 分 |
      | 實作／測試 | ~8 例，`aggregate` 維持純函式 | 同規模＋2 例 | 約 2 倍，需注入時鐘、**破壞純函式** |

      **結論：甲 > 乙 > 丙，丙應排除。** 丙的好處在甲底下本來就同時成立（甲盤中即當下最後成交、
      收盤後自然鎖 13:30，不需切換），付兩倍成本只換到「收盤後由 13:30 變 13:33」＝語意偏好，
      非正確性差異。

      **丙的切換條件兩條路都堵死**（這是排除它的主因，不是「太複雜」）：
      - **牆鐘**：`export async function serveLive` 的 cacheKey 不含 query、`LIVE_TTL` 15s fresh
        ＋stale 120s，而 `ts` 在**建置時**決定 → 13:29:58 建的 payload 最長服務到 13:31:58，
        **實際切換時刻落在 13:30–13:32 之間、不確定**。要「服務時才決定」就得每請求改寫 body，
        快取形同虛設。
      - **資料**（例如「個股 max ≤13:30 即算收盤後」）：循環定義，且「開盤首分鐘、個股 max
        還是昨天」時條件成立 → 判成收盤後走乙，**方向與直覺相反**。

      **⚠️ 三案共通、且是唯一「確定會壞」的下游**：`export function framesDegenerate` 門檻
      `CLOSE_MIN`=13:30。現行收盤後 `ts`≈14:40 → `pickFrames` 選到 13:35 格 → 回 `true`
      （正確標收盤殘影）；`ts` 改成 13:30／13:33 後目標退到 13:2x → 選到 13:20/13:23 格 →
      回 **`false`** → `attachFlowLast` 不附 `flow_last`、收盤定格徽章失效，且該窗 Δ 混入 14:30
      ...
      **其他直接從 `ts` 切日期的地方**（改前必看）：Worker `export function flowLastPayload` 的
      ...
      `flow_last.date` 寫成昨天並被 TTL 保留 7 天**）；`src/build_daysummary.py:141` 閘門
      （**乙在指數列缺失時 exit 0 → daysummary 斷 → 連鎖 LINE ov 卡與 taiwan-stock-news「昨日市場」**，
      這是唯一的跨站路徑）；前端 `index.html:685/1021/1441/2411/2454`。

      **實查確認零衝擊**：`/status` 的 live 走 `async function statusSiteLive` → KV `fi:<date>`，
      **不吃 `ts`**；LINE 圖卡（`buildCardsData`）與姊妹站（`morning.json`／`aetf/latest.json`）
      都不讀 `/live` 的 `ts`；`x-gen` 與前端「延遲」判定吃 `generated_at`；`test/parity.mjs` 不檢查 `ts`。

      **前端頂列副作用**：甲、乙都讓 `index.html` 的 `function liveDataDate`（分支 `if(m[2]>="09:00")`）
      「≥09:00 取 `ts` 日期」分水嶺**恆成立**，`prevWeekday`／`flow_last` 後備走不到
      （**2026-09-08 覆驗更正：當時原文寫「成死碼」不準確**——該後備在 `ts` 為 `null` 時仍會走到，
      走不到的只是「`ts` 存在但 <09:00」這半，且屬推論非保證）。乙若指數列盤中不更新，`d≠today` →
      `function liveStatus` **整個盤中回「休市定格」**。
      （**行號引用已於 2026-09-08 移除**：原寫 `index.html:305`／`:313`，在 `b7be0f0` 就已對不上——
      現行 `:305` 是 `prevWeekday` 的迴圈體、`:313` 是註解行。本檔引用 `index.html` 一律比照對
      `worker/src/index.js` 的既有約定，用**可 grep 唯一命中的宣告字串**，不用行號。）

      **對已裁示的乙案，評估者提出條件性反對（記錄以供裁示）**：乙把方案押在「指數列盤中會更新」
      這個**未觀測**性質上，失敗代價不對稱（盤中產品的頂列整個盤中都錯＋`flow_last.date` 錯一週）；
      甲在兩種世界裡都不會出現這些失效，而它唯一的缺點（秒級退 31 秒）**經實查無任何下游消費者受影響**
      （`pickFrames`／`computeFlow`／前端判定全都只吃分鐘或日期層級）。

      **分岔裁示規則（等 09-08 盤中樣本）**：
      - 盤中 `ts_index.twse` 帶當日**且時分隨採樣時刻推進** → **維持乙**，另訂 `001` 缺失退路，
        並實測 13:25–13:32 的指數列時戳確認 `flowLastPayload.date` 不會寫成昨日。
      - 盤中仍是前一交易日 13:33，或當日但時分不動 → **改採甲**（裁示前提「完全不漂」只在收盤後成立）。
      - 第三種形狀 → 照判準表原樣記錄，不當場定案。
      - 使用者不願再等 → **採甲**。

      **仍為推測、需 09-08 驗證**：指數列 `date` 盤中是否更新、提早收盤／颱風假下的行為、
      FinMind 盤中延遲時指數列是否同步停滯、乙在 13:25–13:32 是否真會寫錯 `flow_last.date`。
      **不確定需查證**：`001`／`101` 何種情況會不一致或缺失（五樣本皆同值且皆存在，但無規則保證）。

      **✅ `flow_last` 的語意轉換已確認（2026-09-09）**：C 案上線當日（09-08）`flow_last.ts` 仍是
      `13:35:08`——那是**舊版程式**在台北 13:25–13:40 寫入窗寫進 KV 的（部署在 23:07，晚於寫入窗），
      當時只能記「下一個交易日的寫入才會是新語意」。09-09 實測 `flow_last.ts` ＝ **`2026-09-09 13:33:00`**
      ＝指數列時戳，`flow_last.date` ＝ `2026-09-09` 正確。**`flowLastPayload` 刻意讀對外 `ts` 的設計
      （非 `snap_ts`）在真實寫入窗運作正常**，先前擔心的「乙案若指數列在 13:25–13:40 尚未更新 →
      `flow_last.date` 寫成昨日」風險，因指數列盤中就在跳而不成立（09-08 盤中兩筆實測落後個股 max 僅 4–5 秒）。

      **仍缺的樣本**：~~非交易日週日~~（樣本 2 已補）；~~真個股會不會脫隊~~（樣本 3–5 已答：
      會，且有興櫃與盤後定價兩種）；**交易日仍需再 4 天**（09-07 是第 1 天，判準表要求 5 個交易日，
      已排 09-08~09-11 每晚 21:00）；**⚠ 盤中樣本 0 筆**——裁示改用指數列後，
      「指數列 `date` 在盤中是否隨盤跳動」成為**擋住動手的關鍵未知**，已排 09-08 盤中兩班。
  - **前端 live 閘門已放寬（2026-09-05，本次上線）**：`index.html` 的 `state.live` 唯一賦值點
    ...
    - **五個 `ts` 下游消費者已逐一 headless 實測不爆**（`ts` 給 `null`／缺欄／空字串三種）：
      `#tsline`（當時顯示「最後成交 —」；**該文案已於 2026-09-08 改為「指數時間」三態，見上方「已結案」段**）、`insightGatherContext`
