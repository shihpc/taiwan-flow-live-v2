  ...
  - **量測管道已建好：`/livediag` 唯讀診斷路徑（2026-09-05，本次上線；C 案本身仍未做）**——
    ...
    - **要觀察什麼／觀察到什麼才能決定改法**（下一個 session 照這張表做）：
      ...
      **✅ 採樣結案：五個交易日全數到齊（2026-09-11 樣本 9，09-07~09-11 每個交易日各一筆收盤後樣本）**
      ...
      | 秒級漂移的幅度範圍？ | **跨日**：`:27`~`:57`＝**30 秒**跨度。**同日內**（唯一有日內序列的 09-07）：`13:30:58`(13:35)→`13:30:58`(14:41)→**`13:30:27`**(21:01)＝**往前掉 31 秒** | 分鐘級穩、秒級不穩。**同日內的漂移不是雜訊而是機制**——盤後成交覆寫掉原本的 max 那列，max 退到次大值（見上方結論②） |
      | `ts_index` 是否每天都是 `13:33:00` 不動？ | **5/5 是**，且 twse／tpex 兩列**同值**；09-07 當天 13:35／14:41／21:01 三次取樣也**逐字相同** | **跨日與日內雙重不動**。結構性理由：指數列不參與個股盤後定價交易，免疫於覆寫機制 |
      ...
      **決策欄（本表原設計是「湊齊後照上表決策」）——決策已於採樣完成前做出**：
      使用者 2026-09-07 裁示採候選二（指數列），C 案已於 2026-09-08 上線（`7f58829`，
      version `aba39e9e`）。**因此樣本 6–9 的角色不是決策輸入，而是上線後的回歸監控**，
      四筆全綠（`ts` 為指數列、`snap_ts` 未外洩、`pickFrames` 仍吃 `snap_ts`）。
