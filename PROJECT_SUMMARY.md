  ...
  **亂序與重複點並不在被接受的範圍內**。
  ...
  - **三件既有狀況（其中兩件已於 2026-09-12 的 B5 修掉，保留為變更紀錄）**：
    ...
    - ~~**「缺欄＝不可比」的舊敘述不精確**：`Number(null)`／`Number("")`／`Number(0)` 皆為
      ...
      **未修的理由要更新**：原寫「沒有資料可以定門檻」——那句對「用 `stale` 的 >3 分」這條路
      仍然成立（近 8 個交易日 `frames[].stale` 實測 **52–54/54**，綁整個 `stale` 會丟掉約 98%
      的序列點）。但**還有一條免門檻的路沒評估**：累計額單調遞增是**結構不變式**，
      「較早時點的 `amt` 大於某個較晚時點」可直接判為污染，不需要任何可調參數。
      該做法會改動 `mergeSeriesPoint` 的語意（要決定丟哪一點），**屬待裁決，不是待實作**。
