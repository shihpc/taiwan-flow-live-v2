    ...
    - **要觀察什麼／觀察到什麼才能決定改法**（下一個 session 照這張表做）：
      ...
      **✅ 已實作（2026-09-08，commit `7f58829`，`worker-deploy` run `34242689680` success，
      ...
      - **下游三處實查結論（非臆測）**：
        ...
        | 前端 `function liveDataDate`（`index.html`，**本批未改**）| **新語意下行為正確**。非交易日 `ts` ＝前一交易日 `13:33`（≥09:00）→ 直接走第一分支取其日期＝正解；`hm<09:00` 的 `flow_last`／`prevWeekday` 後備如預期**走不到**（無害，留作 `ts` 形狀劣化時的降級）
      ——**2026-09-08 覆驗更正：當時寫成「變成死碼」是過強的斷言**。後備分支整體是**活路徑**（`ts` 為 `null` 時
      就會走到它，見本列下一句），走不到的只有「`ts` 存在但時分 <09:00」這個子情形，**且那是推論不是保證**
      （09:00 分水嶺自身即註明為「依兩次觀測的推論」）。`ts === null` → 正則不匹配 → 回 `flow_last.date` 或 `null`，`renderTopline` 顯「—」、`liveStatus` 走「休市定格」，**不拋錯**。**原留給前端那批的兩個文案問題已於 2026-09-08 修畢（commit `af03a9e`＋覆驗退回修正）**：`liveStatus` 的 tooltip 曾寫「最後成交 ${live.ts}」、且 `ts` 為 `null` 時會印出字面 `null`（僅當 `flow_last.date` ＝今日才可能同時成立）——現改「指數時間 ${idxTsText(live)}」，`null` 降級為「—」|
      ...
      **實際措辭決定**：畫面用詞統一為「**指數時間**」；crumb 另掛 `title`
      ...
      改由後備來源推得」——**措辭刻意不提時間比較**，因為否定側同時涵蓋「`ts` 為 truthy 但格式不合、
      `TS_RE` 沒解析成功」的情形，那條從未做過該比較（2026-09-08 覆驗第二點）。
      ...
      **前端頂列副作用**：甲、乙都讓 `index.html:305` 的「≥09:00 取 `ts` 日期」分水嶺**恆成立**，
      `prevWeekday`／`flow_last` 後備走不到（**2026-09-08 覆驗更正：當時原文寫「成死碼」不準確**——
      該後備在 `ts` 為 `null` 時仍會走到，走不到的只是「`ts` 存在但 <09:00」這半，且屬推論非保證）。乙若指數列盤中不更新，`d≠today` → `liveStatus`（`:313`）
      **整個盤中回「休市定格」**。
