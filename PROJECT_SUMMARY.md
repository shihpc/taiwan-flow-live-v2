  ...
  **現況（2026-08-30 更新，原「未解/待觀察」已收斂成「已記載＋待裁決」兩半）**：
  ...
  - **量測管道已建好：`/livediag` 唯讀診斷路徑（2026-09-05，本次上線；C 案本身仍未做）**——
    ...
    - **要觀察什麼／觀察到什麼才能決定改法**（下一個 session 照這張表做）：
      ...
      **⚠️ 前端那批的待辦清單（2026-09-08 覆驗補完，原清單只列了 2 處）**：實查 `index.html`
      消費 `live.ts` 共 **8 處**（`:306`／`:322`／`:323`／`:332`／`:685`／`:873`／`:1021`／`:1441`／`:2411`）。

      | 位置 | 問題 | 優先序 |
      |---|---|---|
      | `:332` `renderTopline` 資料日 tooltip | 仍寫「最後成交 `${live.ts}`（上游時戳，**非資料日依據**）」——**新語意下自我矛盾**，因為 `liveDataDate`（`:309`）正是拿 `ts` 的日期當資料日 | **最高**（可見的錯誤敘述）|
      | `:873` 摘要分析 crumb | 「最後成交 `${state.live.ts}`」是**可見文字**不是 tooltip | 高 |
      | `:322`／`:323` `liveStatus` tooltip | 仍寫「最後成交」；且 `ts` 為 `null` 時會印字面 `null` | 中 |
      | `:685`／`:1021`／`:1441`／`:2411` | **無正確性問題**——都取日期部分、都有 `||""` 守門，且**新語意下更正確**（例：`:2411` 的 `/replay?date=` 舊語意在週末會拿週六日期去打） | 不必改 |
